...
public sealed partial class InteractionArgs
{
    ...
    /// </remarks>
    public Dictionary<string, object> Blackboard => _boardStorage ??= new(3);
    private Dictionary<string, object>? _boardStorage; // null by default, allocated lazily (only if actually needed)
    ...
    public InteractionArgs(InteractionArgs src) : this(src.User, src.Target, src.Used, src.CanAccess, src.CanInteract, src.HasHands, src.ContestAdvantage) {}
    ...
    {
        value = default;
        if (_boardStorage == null || !_boardStorage.TryGetValue(key, out var rawVal))
            return false;
