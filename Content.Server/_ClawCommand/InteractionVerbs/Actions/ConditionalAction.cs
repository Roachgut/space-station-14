...
public sealed partial class ConditionalAction : InteractionAction
{
    ...
    private InteractionAction? ResolveBranch(InteractionArgs ctx, InteractionVerbPrototype proto, VerbDependencies deps)
    {
        return Condition.IsMet(ctx, proto, deps) ? TrueBranch : FalseBranch;
    }
    ...
    public override bool IsAllowed(InteractionArgs ctx, InteractionVerbPrototype proto, VerbDependencies deps)
    {
        ...
        var chosen = ResolveBranch(ctx, proto, deps);
        return chosen?.IsAllowed(ctx, proto, deps) ?? !FailWhenNoBranch;
    }
    ...
    public override bool CanPerform(InteractionArgs ctx, InteractionVerbPrototype proto, bool beforeDelay, VerbDependencies deps)
    {
        ...
        var chosen = ResolveBranch(ctx, proto, deps);
        return chosen?.CanPerform(ctx, proto, beforeDelay, deps) ?? !FailWhenNoBranch;
    }

    public override bool Perform(InteractionArgs ctx, InteractionVerbPrototype proto, VerbDependencies deps)
    {
        var chosen = ResolveBranch(ctx, proto, deps);
        return chosen?.Perform(ctx, proto, deps) ?? !FailWhenNoBranch;
    }
}
